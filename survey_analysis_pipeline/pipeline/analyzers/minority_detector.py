...
RELEVANCE_PROMPT = """以下の意見が「{survey_title}」に関連する法案検討において、**政策立案者が参照すべき具体的かつ重要な知見**を含んでいるか、極めて厳格に判定してください。

## 意見
{content}

## 厳格な除外基準（relevant: false）- 以下は全て除外
1. **挨拶・謝辞・激励・感想**（「ありがとう」「勉強になった」「難しい」等）
2. **システム・AIへのメタコメント**（「AIすごい」「質問が長い」等）
3. **内容が空虚・不明瞭**（「特になし」「わからない」「あ」等）
4. **テーマと無関係な雑談**（自分語り、全く別の話題）
5. **抽象的すぎる一般論**（「国民のために頑張って」「改革が必要だ」等の具体性がない応援）
6. **既出の一般的意見**（単なる「賛成」「反対」のみで、独自の視点や具体的根拠がないもの）

## 採用基準（relevant: true）- 以下の**高い基準**を満たす場合のみ採用
- 政策立案者が「なるほど」と唸るような独自の視点がある
- 具体的な実務経験や現場の事例に基づいている
- 法案の盲点や具体的なリスクを指摘している

## 出力形式
JSON形式で出力してください：
{{"relevant": true または false, "reason": "判定理由（具体的に）"}}
"""
